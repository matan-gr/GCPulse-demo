...
export const SkeletonCard: React.FC<{ viewMode: 'grid' | 'list' }> = ({ viewMode }) => {
  ...
  return (
    <div className={`bg-white dark:bg-slate-800 rounded-2xl shadow-sm border border-slate-200 dark:border-slate-700/50 overflow-hidden flex ${isList ? 'flex-row h-[200px]' : 'flex-col h-[450px]'}`}>
      <div className={`${isList ? 'w-1/3' : 'h-48'} bg-slate-200 dark:bg-slate-700 animate-pulse`} />
